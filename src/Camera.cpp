...
Camera::Camera(CVector3 vZero, CVector3 vView, CVector3 vUp){
...
void Camera::cameraControl()
{
	Look();
	//if(m_frame<3)
	//	navigate(posArray2[5],posArray2[5],0,1000,0,0);
...
void Camera::loadToFIle(const char * fileName)
{
	...
	ifstream  file;
	file.open(fileName);
	file >> m_vPosition.x >> m_vPosition.y >> m_vPosition.z ;
	file >> m_vView.x >> m_vView.y >> m_vView.z;
	file.close();
}
