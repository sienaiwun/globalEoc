...
RT_PROGRAM void mesh_intersect(int primIdx)
{
	...
	if (intersect_triangle(ray, p0, p1, p2, n, t, beta, gamma)) {
		...
		if (rtPotentialIntersection(t)) {
			...
			float2 t0 = vertex_buffer[v_idx.x].tex;
			float2 t1 = vertex_buffer[v_idx.y].tex;
			float2 t2 = vertex_buffer[v_idx.z].tex;
			//  texcoord = make_float3(1,0,0);
			
			texcoord = make_float3((t0*(1.0f - beta - gamma) + t1*beta + t2*gamma),1);
