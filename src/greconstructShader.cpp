...
void GReconstructShader::setScene(Scene * pScene)
{
	glUniform3fv(m_lightPosBinding, 1, (float*)pScene->getLightPos());
}
static int objectId = 1;
void GReconstructShader::setGeomtryIndex(int i)
{
	glUniform1i(m_objectId, i);
}
void GReconstructShader::begin()
{
	m_loader.useShder();
	glUniform3fv(m_objectDiffuseBinding, 1, m_diffuseColor);

}
void GReconstructShader::end()
{
	m_loader.DisUse();
	objectId = 1;
}

void GReconstructShader::setMaterial(const GLMmaterial & material, textureManager & manager)
{
...
void GReconstructShader::setCamera(Camera *pCamera)
{
